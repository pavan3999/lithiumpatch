...
6. Run `go generate ./app` from the root of the repository to download the APK. If this does not work, you can manually download the Lithium 0.25.0 APK from [here](https://www.apkmirror.com/apk/faultexception/lithium-epub-reader/lithium-epub-reader-0-25-0-release/lithium-epub-reader-0-25-0-android-apk-download/) or extract it from your device.
7. Run `go run . app/Lithium_0.25.0.apk` from the root of the repository. Use `--help` to see additional options including using a custom keystore, setting the tool paths, and adding fonts from an external directory.
...
options:
  ...
  -d, --diff string                  Write diff to the specified file (default: disabled)
      ...
      --apktool string               Path to apktool.jar (3.0.3) (default "lib/apktool-3.0.3.jar")
...
**Note:** If you get an error from apktool about `No resource identifier found for attribute 'preserveLegacyExternalStorage'`, run `java -jar lib/apktool-3.0.3.jar clean-frameworks`.
